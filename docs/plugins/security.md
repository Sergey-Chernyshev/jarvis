...
This commit supplies non-activating package services. The guarantees below
are exercised by package/receipt/trust tests; production runtime admission,
launchd lifecycle, and the manager CLI are subsequent integration work. The
existing protocol-1 sidecar path is not covered by v2 receipts in this stage.
