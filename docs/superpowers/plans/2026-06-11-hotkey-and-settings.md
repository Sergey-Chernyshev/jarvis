...
```js
function showPanel() {
  // тихий режим: трей, клик по уведомлению
...
function showPanelFocused() {
  // raycast-режим: хоткей
  if (!panel || panel.isDestroyed()) createPanel();
  panelFocusMode = true;
  positionPanel();
  panel.show();
...
function registerHotkey(accelerator) {
  ...
  let ok = false;
  try {
    ok = globalShortcut.register(accelerator, toggleHotkeyPanel);
  } catch {}
  if (!ok && accelerator !== current) {
    try {
      globalShortcut.register(current, toggleHotkeyPanel);
    } catch {}
