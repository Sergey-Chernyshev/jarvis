...
- ✅ **Фаза 3** — control/settings: sessions.reply, sessions.control, settings.set. Общие ядра вынесены из `ipc.rs` (no dup). Приёмочные 2, 3 (структурно). _Отложено:_ sessions.queue/launch/interrupt (новая инфраструктура).
