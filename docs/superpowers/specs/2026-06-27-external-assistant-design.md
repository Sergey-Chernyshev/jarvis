...
**4b — выполнение команд через permission-tool + карточку (опасное):** 4. `capability/native/permission.rs` + регистрация в `jarvis-mcp` — permission-тул
(read→allow; side-effect→confirm-карточка; неизвестное→deny). Чистый
классификатор read-vs-side-effect (тест). 5. AssistantHost: добавить `--permission-prompt-tool mcp__jarvis__permission` +
полный тул-набор (Bash/Write/Edit). Confirm-карточка показывает точную команду. 6. Безопасность-тесты: side-effect недостижим без confirm; deny по умолчанию;
