...
- **Не делаем в v1**: отправку ввода агенту в VM (`session_reply`). У avm нет attach-примитива;
  путь через собственный tmux в госте (`limactl shell … tmux …`) — отдельная фича после
  обкатки монитора (соответствует «монитор/пульт, не оркестрация»).
...
2. **PluginHost**: discovery/манифест/enable/spawn/handshake/supervise; обобщение
   `plugins_status`/`plugins_cmd`; токен-выпуск. Фейк-плагин в тестах.
3. **Раздел «Плагины»** в settings2.js: список + деталь + схема-рендер + гранты.
4. **agent-vm v1 (монитор)**: инвентарь + runtime-статус → сущности `vm.*`, тосты/TTS, трей.
   Закрывает минимум issue #44.
5. **agent-vm v2 (телеметрия)**: hook-инжект, TCP-мост, `sessions.ingest`,
   liveness-провайдер, бейдж VM в списке сессий.
6. **SkillRegistry** + скилы плагинов (`vm_status`, `vm_power` с confirm).
7. **Меж-плагинные `consumes`-гранты** (UI + фильтрация query/subscribe) — активируется, когда
   появится второй плагин-потребитель.

Инкременты 1–4 самодостаточны; 5–7 включаемы независимо.
...
## 8. Открытые вопросы (не блокируют инкременты 1–3)

1. Язык/размещение плагина agent-vm: предложен Rust-крейт в этом репо (+ выделение
   `jarvis-plugin-sdk`). Альтернатива — отдельный репозиторий сразу; тогда SDK нужно
   публиковать раньше.
2. Нужен ли `vm_create` голосом/из UI (создание VM, а не только мониторинг имеющихся) — или
   это уже «оркестрация» и вне границ проекта?
3. Порог для варианта C (UI-плагины в webview): ждём реального запроса на кастомные панели или
   закладываем `panels` в манифест сразу (в v1 — нет)?
