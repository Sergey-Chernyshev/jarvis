...
1. Язык/размещение плагина agent-vm: предложен Rust-крейт в этом репо (+ выделение
   ...
   публиковать раньше.
2. Создание VM при первом запуске: предложено «авто с подтверждением» (диалог + прогресс).
   Альтернатива строже — только запуск в уже созданные руками VM (создание отложить).
3. Хуки для codex в VM: модуль codex у avm хуков не ставит. Вариант-расширение — свой
   пользовательский модуль `~/.config/agent-vm/modules.d/jarvis.sh`, ставящий hook-конфиги
   обоих агентов в гостя (но он попадает только в VM, созданные с этим модулем). В MVP
   телеметрия codex-сессий из VM отсутствует — приемлемо?
4. Порог для варианта C (UI-плагины в webview): ждём реального запроса на кастомные панели или
