...
Дата: 2026-07-03 · Ветка: `feat/agent-vm-plugin` · Статус: архитектурная база

> Продуктовый сценарий agent-vm пересмотрен 2026-07-28: Jarvis теперь не только
> открывает терминал, а сам управляет VM-агентом, транслирует чат и результаты в
> UI. Актуальная v2-спека:
> [`2026-07-28-agent-vm-project-runtime-design.md`](2026-07-28-agent-vm-project-runtime-design.md).
> Этот документ остаётся источником решений по PluginHost, EntityStore и модели
> прав плагинов; конфликтующие пользовательские сценарии заменены v2.
