...
allowed_trust_verifier="$repo_root_resolved/src-tauri/src/plugin_packages/trust/package.rs"
...
if [[ "${#trust_roots[@]}" -gt 0 ]] && ! trust_scan="$(
  ...
  failed=1
  # Preserve the bounded partial diagnostics when the scanner fails closed.
fi
...
forbidden_plugin_imports="$(
  rg -n --no-heading '(src_tauri|jarvis::daemon|jarvis::(?:plugins|plugin|plugin_packages))' \
