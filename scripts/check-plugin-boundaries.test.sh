...
write_clean_fixture
mkdir -p "$fixture_root/src-tauri/src/plugin_packages"
printf '%s\n' \
  'struct WrongVerifier;' \
  'impl PackageTrustVerifier for WrongVerifier {}' \
  > "$fixture_root/src-tauri/src/plugin_packages/wrong.rs"
expect_rejected "PackageTrustVerifier production implementation outside host trust adapter"
...
write_clean_fixture
mkdir -p "$fixture_root/src-tauri/src/plugin_packages/trust"
printf '%s\n' \
  'pub use jarvis_package::PackageTrustVerifier as CrossRootTrust;' \
  > "$fixture_root/src-tauri/src/plugin_packages/trust/package.rs"
printf '%s\n' \
...
printf '%s\n' \
  'pub mod trust;' \
  'mod wrong;' \
  > "$fixture_root/src-tauri/src/plugin_packages/mod.rs"
printf '%s\n' \
  'pub mod package;' \
  > "$fixture_root/src-tauri/src/plugin_packages/trust/mod.rs"
printf '%s\n' \
  '#[macro_export]' \
  'macro_rules! r#type {' \
...
write_clean_fixture
mkdir -p \
  "$fixture_root/src-tauri/src/plugin_packages/trust" \
  "$fixture_root/src-tauri/src/plugin_packages"
printf '%s\n' \
  '#[macro_export]' \
  'macro_rules! catalog_verifier {' \
  '    ($type:ty) => { impl PackageTrustVerifier for $type {} };' \
...
node -e '
  ...
  const manifests = [
    ...
    "tools/plugin-schema-parity/Cargo.toml",
...
for private_module in plugin plugin_packages; do
  write_clean_fixture
  printf 'use jarvis::%s::PrivateHostType;\n' "$private_module" > "$fixture_root/plugins/community/src/lib.rs"
  expect_rejected "plugin source imports a Jarvis Core implementation module"
done
