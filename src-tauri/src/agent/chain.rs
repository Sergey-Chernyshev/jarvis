...
/// Потолок глубины авто-цепочки ПО УМОЛЧАНИЮ. Типовой цикл «доведи до зелёного» —
/// 3–5 заходов; десять дают двойной запас и при ходе в 5–10 минут это примерно
/// час работы без взгляда человека. Дальше цепочка встаёт САМА и говорит словами.
///
/// Значение оставлено прежним по решению владельца: «потолок точно снижать не
/// будем». Настройкой его теперь можно поднять или опустить — константа без
/// ручки была дырой, которую я сам числил в известных.
pub const MAX_STEPS: u32 = 10;

/// Разумная верхняя граница ручки. Не запрет, а защита от опечатки: «100» вместо
/// «10» — это сто пробуждений Джарвиса подряд, каждое с полным контекстом чата.
const MAX_STEPS_LIMIT: u32 = 50;

/// Потолок глубины из настроек (`autonomy.maxSteps`), с прежним умолчанием.
///
/// Читается на КАЖДОЕ решение, а не запоминается на старте: человек правит
/// потолок ровно тогда, когда цепочка ему мешает, и ждать перезапуска в этот
/// момент — издевательство.
pub fn max_steps(s: &Value) -> u32 {
    s.pointer("/autonomy/maxSteps")
        .and_then(Value::as_u64)
        .filter(|v| *v > 0)
        .map(|v| (v as u32).min(MAX_STEPS_LIMIT))
        .unwrap_or(MAX_STEPS)
}
...
pub struct Chains {
    ...
    /// Действующий потолок глубины. Обновляется из настроек перед каждым
    /// решением (`on_session_done`), а не читается при запуске: человек правит
    /// потолок ровно тогда, когда цепочка ему мешает.
    ///
    /// Поле, а не аргумент, сознательно: `on_done` зовут два десятка проверок, и
    /// протаскивать через них число значило бы менять двадцать две строки ради
    /// одной. Умолчание — прежнее, поэтому в тестах ничего не меняется.
    cap: std::sync::atomic::AtomicU32,
}
...
impl Default for Chains {
    fn default() -> Self {
        Chains {
            ...
            cap: std::sync::atomic::AtomicU32::new(MAX_STEPS),
        }
    }
}
...
impl Chains {
    ...
    /// Поставить действующий потолок глубины (из настроек).
    pub fn set_cap(&self, cap: u32) {
        self.cap.store(cap.max(1), std::sync::atomic::Ordering::Relaxed);
    }

    /// Действующий потолок — для текста отказа: человек должен видеть ТО число,
    /// по которому его остановили, а не умолчание из кода.
    pub fn cap(&self) -> u32 {
    ...
    pub fn on_done(&self, session_id: &str, at: i64) -> Vec<(String, Decision)> {
        ...
        for (chat_id, c) in m.iter_mut() {
            ...
            let d = match c.mode {
                ...
                Mode::Auto if c.step >= self.cap.load(std::sync::atomic::Ordering::Relaxed) => {
...
pub fn on_session_done(d: &Arc<Daemon>, session_id: &str, at: i64) {
    morning_check(&d.app);
    // Потолок глубины — из настроек и ПЕРЕД решением: человек правит его ровно
    // тогда, когда цепочка ему мешает, и ждать перезапуска в этот момент —
    // издевательство.
    chains().set_cap(max_steps(&d.settings.load()));
...
async fn run_step(d: &Arc<Daemon>, chat_id: &str, sid: &str, decision: Decision) {
    ...
    if let Decision::Depth = decision {
        emit(
            ...
            json!({
                "reason": "depth",
                // Число берём ДЕЙСТВУЮЩЕЕ: человек должен видеть тот потолок,
                // по которому его остановили, а не умолчание из кода.
                "text": format!(
                    "Потолок авто-цепочки — {} заходов подряд (настройка autonomy.maxSteps). \
                     Дальше нужен твой взгляд",
                    chains().cap()
                ),
            }),
        );
    ...
    match decision {
        Decision::Propose => {
            let prompt = wake_message(&outcome, sid, chains().next_step(chat_id));
        ...
        Decision::Send(step) => {
            ...
            let prompt = wake_message(&outcome, sid, step);
            ...
            if wake(d, chat_id, sid, &prompt, step).await.is_ok() {
...
/// Разбудить чат Джарвиса событием «сессия закончила ход».
///
/// Раньше здесь цепочка писала промпт ПРЯМО В СЕССИЮ, минуя Джарвиса. Отсюда
/// брались два диспетчера на одну сессию и сообщения, которых Джарвис не
/// узнавал: он их и правда не писал.
///
/// Теперь диспетчер один. Цепочка не решает, что делать с сессией, — она
/// доставляет событие тому, у кого есть контекст: чату, который эту сессию
/// ведёт. Дальше Джарвис делает ОБЫЧНЫЙ ход в своей ленте и сам решает,
/// отправить ли следующий промпт (тогда это будет его `sessions.reply` со своей
/// пометкой происхождения), доложить человеку, остановиться или спросить.
///
/// Ход Джарвиса — это новый запуск CLI с `--resume`, а не запись в живой
/// процесс, поэтому будить есть чем и приёмника внутри хода не требуется. Но
/// второй ход по той же нити одновременно запускать нельзя: две копии на одном
/// `--resume` испортят разговор. Идёт ход — отступаем и говорим об этом.
pub(crate) async fn wake(
    ...
    let app = d.app.clone();
    // Нить чата: по ней же проверяется занятость.
    let thread = crate::agent::chat_book(&app)
        .chats
        .iter()
        .find(|c| c.id == chat_id)
        .and_then(|c| c.session_id.clone());
    if let Some(t) = thread.as_deref() {
        if crate::agent::turn_in_flight(t) {
            let text = "Джарвис сейчас занят своим ходом — пробуждение отложено, \
                        событие не потеряно";
            refuse(&app, chat_id, "busy", text);
            return Err(text.to_string());
        }
    }
    let out = crate::ipc::agent_send(
        app.clone(),
        message.to_string(),
        None,
    ...
    if out.get("ok").and_then(Value::as_bool).unwrap_or(false) {
        ...
        // Событие доставлено — значит утреннее «ждёт тебя» по этому чату
        // отработано: висящая карточка после решения хуже её отсутствия.
        night_log().resolve(chat_id);
        emit(
            &app,
            chat_id,
            "sent",
            json!({ "sessionId": sid, "prompt": message, "step": step }),
        );
        return Ok(());
    }
    let text = format!(
        "Джарвиса не удалось разбудить: {}",
        out.get("error")
            .and_then(Value::as_str)
            .unwrap_or("чат не принял событие")
    );
    refuse(&app, chat_id, "wake", &text);
    Err(text)
}

/// Сообщение, которым будят чат Джарвиса. ФАКТЫ СОБЫТИЯ, а не сочинённая задача.
///
/// Здесь раньше стоял отдельный сочинитель: боковой вызов Haiku, который видел
/// ТОЛЬКО итог последнего хода. Он не видел ни задачи человека, ни переписки
/// чата, ни предыдущих заходов — и физически не мог знать, закончена работа или
/// нет. Отсюда и выдуманная работа (модели велели написать следующий шаг, она
/// обязана была что-то написать), и уход сессии в чужую тему (увидев в общей
/// сессии чужой итог, он честно продолжал ЕГО).
///
/// Решать, что дальше, должен тот, у кого есть контекст: чат Джарвиса, которому
/// человек поставил задачу. Поэтому мы не сочиняем заход, а отдаём событие и
/// право решать.
fn wake_message(o: &Outcome, sid: &str, step: u32) -> String {
    let mut p = format!(
        "Сессия {sid} закончила ход. Это пробуждение по событию, а не сообщение человека \
         (заход {step} из {MAX_STEPS} — потолок глубины, а не план).\n\n"
    );
    if !o.summary.is_empty() {
        p.push_str(&format!("Итог хода: {}\n", o.summary));
    }
    if !o.files.is_empty() {
        let list: Vec<&str> = o.files.iter().take(8).map(|f| f.path.as_str()).collect();
        p.push_str(&format!("Тронуто: {}\n", list.join(", ")));
    ...
    match &o.tests {
        Some(t) if !t.ok => p.push_str(&format!("Тесты КРАСНЫЕ: {}\n", t.line)),
        Some(t) => p.push_str(&format!("Тесты зелёные: {}\n", t.line)),
        None => p.push_str("Проверку в этом ходе не прогоняли.\n"),
    }
    if !o.reply.is_empty() {
        p.push_str(&format!("\nОтвет агента:\n{}\n", o.reply));
    }
    p.push_str(
        "\nСверь это с задачей, которую тебе поставил человек, и реши САМ: отправить этой \
         сессии следующий промпт, доложить человеку, остановиться или спросить его. \
         Следующий шаг придумывать не надо — он должен следовать из задачи человека, \
         а не из последней детали в отчёте. Работа закончена — так и скажи словами \
         «продолжать нечего» и новую не начинай.",
    );
    p
}
...
mod tests {
    ...
    /// Отдельного сочинителя заходов быть не должно.
    ///
    /// Здесь стоял боковой вызов служебной модели, который видел ТОЛЬКО итог
    /// последнего хода — ни задачи человека, ни переписки чата, ни предыдущих
    /// заходов. Он не мог знать, закончена работа или нет, и потому выдумывал
    /// её, а увидев в общей сессии чужой итог — честно продолжал его. Решение
    /// человека: решать должен тот, у кого есть контекст, то есть чат Джарвиса.
    ///
    /// Сторож по исходнику, а не по поведению: возврат такого вызова заметен
    /// прогоном, а не месяцем странных заходов в чужих сессиях.
    #[test]
    fn the_chain_never_composes_a_visit_by_itself() {
        let src = include_str!("chain.rs");
        let body = &src[..src.find("#[cfg(test)]").unwrap_or(src.len())];
        assert!(
            !body.contains("run_service_llm"),
            "вернулся боковой сочинитель заходов: у него нет задачи человека"
        );
        // Цепочка больше не ПИШЕТ в сессию сама — она будит чат, у которого есть
        // контекст. Второй диспетчер на одну сессию появлялся именно так.
        //
        // Признак — вызов гейта, а не упоминание имени: имя `sessions.reply`
        // законно встречается в наблюдателе, который по ЧУЖИМ вызовам узнаёт,
        // за какой сессией следить. Проверять по упоминанию значило бы запретить
        // цепочке замечать работу Джарвиса.
        assert!(
            !body.contains("via_gate_panel("),
            "цепочка снова пишет в сессию мимо Джарвиса"
        );
        assert!(body.contains("crate::ipc::agent_send("), "пробуждение чата пропало");
    }

    /// Сообщение пробуждения несёт ФАКТЫ и право решать, а не готовую задачу.
    #[test]
    fn the_wake_message_hands_over_facts_and_the_right_to_decide() {
        let o = Outcome {
            session_id: "s1".into(),
            summary: "починил парсер".into(),
            reply: "готово".into(),
            ..Default::default()
        };
        let m = wake_message(&o, "s1", 3);
        assert!(m.contains("починил парсер"), "итог не передан");
        assert!(m.contains("пробуждение по событию"), "не сказано, что это не человек");
        // Главное: решение остаётся за Джарвисом, и он обязан сверяться с задачей.
        assert!(m.contains("реши САМ"), "право решать не передано");
        assert!(
            m.contains("задачей, которую тебе поставил человек"),
            "не сказано, с чем сверяться — вернётся выдуманная работа"
        );
        assert!(
            m.contains("Следующий шаг придумывать не надо"),
            "не запрещено сочинять — ровно это и чиним"
        );
        assert!(m.contains(&format!("из {MAX_STEPS}")), "потолок глубины не назван");
    }
    ...
    fn a_background_pass_asks_the_budget_before_it_goes() {
        ...
        assert!(
            body.find("budget_gate(").unwrap() < body.find("wake_message(").unwrap(),
