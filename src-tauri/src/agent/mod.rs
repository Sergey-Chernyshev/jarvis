...
const AGENT_SYSTEM_PROMPT: &str =
    "Ты — ассистент Jarvis. Используй только предоставленные MCP-инструменты. \
     ...
     kimi, он дешевле на два порядка; 4) занятую сессию не заваливай \
     ...
     карточку. Потолок одновременных сессий — настройка sessionsSpawnMax, по \
     умолчанию 4; при упоре придёт отказ с числами. sessions.close закрывает \
...
mod tests {
    ...
    fn preamble_covers_key_facts() {
        ...
        assert!(AGENT_SYSTEM_PROMPT.contains("sessionsSpawnMax"), "нет потолка подъёма");
