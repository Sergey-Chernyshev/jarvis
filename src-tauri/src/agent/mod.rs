...
pub enum AgentEvent {
    ...
    Failed { message: String, #[serde(default, skip_serializing_if = "String::is_empty")] session_id: String, #[serde(default)] lost_session: bool },
...
pub fn parse_stream_line(line: &str) -> Vec<AgentEvent> {
    ...
    match typ {
        ...
        "result" => {
            ...
            if v.get("is_error").and_then(Value::as_bool).unwrap_or(false) || v.get("subtype").and_then(Value::as_str).is_some_and(|s| s.starts_with("error")) {
                return vec![AgentEvent::Failed {
                 ...
                 session_id: v.get("session_id").and_then(Value::as_str).unwrap_or_default().into(),}];
            }
            let result = v
                .get("result")
                .and_then(Value::as_str)
...
impl StreamLifecycle {
    ...
    pub(crate) fn fail(&mut self, message: impl Into<String>) -> Option<AgentEvent> {
        self.accept(AgentEvent::Failed { message: message.into(), session_id: String::new(), lost_session: false,})
    }
}
...
pub(crate) async fn finish_cli_stream(
    ...
    let message = match tokio::time::timeout(std::time::Duration::from_secs(2), child.wait()).await {
        ...
        Err(_) => {
            stop::kill_tree(child).await;
            format!("{provider} закрыл поток до завершения ответа")
        }
        _ => format!("{provider} не прислал результат. Можно повторить запрос."),
    };
    if let Some(event) = state.fail(read_error.unwrap_or(message)) { emit_event(app, chat_id, &event); }
}
    ...
    pub async fn run(
        ...
        let mut child = match command.spawn() {
            ...
            Err(e) => {
                ...
                self.fail(&format!("Не удалось запустить Claude: {e}"));
                return;
            }
        };
        ...
        finish_cli_stream(&app, &self.chat_id, &mut child, &mut state, "Claude", None).await;
    }
...
mod tests {
    ...
    fn failed_claude_result_is_not_success() {
        ...
        assert_eq!(events, vec![AgentEvent::Failed { message: "Authentication required".into(), session_id: "s2".into(), lost_session: false,}]);
    }
    ...
    fn every_outgoing_event() -> Vec<AgentEvent> {
        vec![
            ...
            AgentEvent::Failed { message: "агент оборвался".into(), lost_session: true, session_id: String::new(),},
            AgentEvent::Failed { message: "claude не найден".into(), lost_session: false, session_id: String::new(),},
            AgentEvent::Stopped {
                by: "user".into(),
    ...
    fn tag_adds_a_field_and_does_not_touch_the_rest() {
        ...
        let failed = AgentEvent::Failed { message: "нет разговора".into(), lost_session: true, session_id: String::new(),};
        assert_eq!(
