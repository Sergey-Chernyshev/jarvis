...
mod tests {
    ...
    /// Агент не правит собственные инструкции. Никогда.
    ///
    /// `agentPreamble` — дописка человека к преамбуле, то есть текст, по которому
    /// агент решает, что ему делать. Дай он себе право его менять — и это дыра
    /// того же класса, что подтверждение собственной карточки: подпись человека
    /// стоит под тем, чего человек не писал. Живой случай был: попытка положить
    /// туда правило гейт отклонил, и правильно сделал.
    ///
    /// Список deny-by-default, так что сегодня ключ закрыт самим отсутствием.
    /// Тест стоит здесь не «на всякий случай», а против удобной правки в
    /// будущем: добавить строчку в allowlist — дело одной секунды.
    #[test]
    fn the_agent_can_never_edit_its_own_instructions() {
        assert!(
            !SETTINGS_ALLOWLIST.contains(&"agentPreamble"),
            "агент получил право переписывать собственную преамбулу"
        );
        // И ни один ключ, начинающийся с `agentPreamble` (например, будущий
        // `agentPreambleExtra`), — обход через соседнее имя тоже закрыт.
        assert!(
            !SETTINGS_ALLOWLIST.iter().any(|k| k.starts_with("agentPreamble")),
            "право на преамбулу проехало под соседним именем"
        );
    }
}
