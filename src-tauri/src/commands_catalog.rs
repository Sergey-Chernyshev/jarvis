...
/// Встроенные команды интерактивной Kimi-сессии — весь реестр Kimi Code CLI 0.37
/// (`BUILTIN_SLASH_COMMANDS`), 40 штук, с его же подсказками аргументов.
///
/// `/hooks` здесь нет намеренно: такая команда была у legacy `kimi-cli`, а в
/// Kimi Code хуки настраиваются только конфигом. Показать её значило бы
/// пообещать человеку то, чего CLI не примет.
/// model/effort с подсказкой — как у Claude: Jarvis открывает свой пикер
/// значений вместо интерактивного слайдера TUI.
const KIMI_BUILTINS: [(&str, &str, &str); 40] = [
    ("yolo", "Авто-апрув действий; вопросы агент всё ещё задаёт", ""),
    ("auto", "Полная автономия: агент не спрашивает вообще", ""),
    ("permission", "Выбрать режим разрешений", ""),
    ("settings", "Открыть настройки TUI", ""),
    ("plan", "Переключить режим плана", ""),
    ("swarm", "Режим роя или одна задача роем", "[on|off] | ‹задача›"),
    ("model", "Сменить модель сессии", "‹выбрать›"),
    (
        "secondary-model",
        "Модель для субагентов (экспериментальная возможность)",
        "",
    ),
    ("effort", "Уровень рассуждения", "‹выбрать›"),
    ("provider", "Провайдеры LLM: добавить, удалить, обновить", ""),
    ("btw", "Спросить отпочкованного бокового агента", "‹вопрос›"),
    ("help", "Список команд и горячих клавиш", ""),
    ("new", "Начать свежую сессию в этой же папке", ""),
    ("sessions", "Просмотреть и возобновить сессии", ""),
    ("tasks", "Просмотреть фоновые задачи", ""),
    ("mcp", "Состояние MCP-серверов", ""),
    ("plugins", "Управление плагинами", ""),
    ("add-dir", "Добавить рабочую папку или показать список", "[list] | ‹путь›"),
    ("experiments", "Управление экспериментальными возможностями", ""),
    ("reload", "Перечитать сессию, config.toml и tui.toml", ""),
    ("reload-tui", "Перечитать только настройки интерфейса", ""),
    ("compact", "Сжать контекст разговора", "‹указание›"),
    (
        "goal",
        "Поставить или вести автономную цель",
        "[status|pause|resume|cancel|replace|next] | ‹цель›",
    ),
    ("init", "Разобрать код и создать AGENTS.md", ""),
    ("fork", "Скопировать сессию, не переключаясь в копию", ""),
    ("title", "Задать или показать заголовок сессии", "‹заголовок›"),
    ("usage", "Токены сессии, окно контекста и квоты плана", ""),
    ("status", "Состояние сессии и рантайма", ""),
    ("feedback", "Отправить обратную связь команде Kimi Code", ""),
    ("undo", "Забрать последний промпт из транскрипта", ""),
    ("editor", "Внешний редактор для Ctrl-G", ""),
    ("theme", "Тема терминального интерфейса", ""),
    ("logout", "Выйти из настроенного провайдера", ""),
    ("login", "Выбрать платформу и авторизоваться", ""),
    ("export-md", "Выгрузить сессию в Markdown", ""),
    ("export-debug-zip", "Выгрузить сессию отладочным ZIP-архивом", ""),
    ("copy", "Скопировать последний ответ ассистента", ""),
    ("web", "Открыть сессию в веб-интерфейсе", ""),
    ("exit", "Выйти из приложения", ""),
    ("version", "Показать версию", ""),
];

pub fn kimi_builtins() -> Vec<Command> {
    KIMI_BUILTINS
        .iter()
        .map(|(name, description, hint)| Command {
            name: (*name).into(),
...
/// Каталог Kimi-сессии.
///
/// Только встроенное. Каталога пользовательских промптов-команд, аналогичного
/// `~/.claude/commands` или `~/.codex/prompts`, у Kimi Code НЕТ: пользовательское
/// расширение там — skills, у них своя механика (обнаружение по каталогам,
/// активация моделью, а не разворачивание текста по `/имя`). Сканировать под них
/// каталог значило бы показать в палитре команды, которых CLI не примет.
pub fn kimi_commands() -> Vec<Command> {
    dedupe(kimi_builtins())
}
...
mod tests {
    ...
    /// У Kimi effort отдельный (в отличие от Codex), а `/hooks` не существует:
    /// показать её значило бы пообещать то, чего CLI не примет.
    #[test]
    fn kimi_builtins_match_the_cli_registry() {
        let out = kimi_commands();
        assert_eq!(out.len(), 40, "реестр Kimi Code 0.37 — ровно 40 команд");
        assert!(out.iter().all(|c| c.source == "kimi"));
        for name in ["yolo", "auto", "plan", "effort", "swarm", "export-debug-zip"] {
            assert!(out.iter().any(|c| c.name == name), "нет /{name}");
        }
        assert!(!out.iter().any(|c| c.name == "hooks"), "/hooks — из legacy kimi-cli");
        // Jarvis подменяет слайдеры TUI своим пикером — по этой подсказке.
        assert_eq!(out.iter().find(|c| c.name == "model").unwrap().hint, "‹выбрать›");
        assert_eq!(out.iter().find(|c| c.name == "effort").unwrap().hint, "‹выбрать›");
        assert!(out.iter().all(|c| !c.description.is_empty()), "команда без описания");
    }
