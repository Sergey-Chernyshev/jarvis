...
pub async fn run_pipeline(
    ...
    // Adopt the continuation persisted by pre-BPMN releases.
    if run.vars.is_empty() { run.vars = run.pipeline_vars.clone(); }
    if run.tokens_at.is_empty() {
        if let Some(node) = run.ask.as_ref().map(|ask| ask.step.clone()).filter(|step| !step.is_empty()).or_else(|| run.pipeline_next.clone()) {
            run.tokens_at.push(TokenState { node, from: String::new(), lane: String::new() });
        }
    }
    ...
    if let Some(a) = run.ask.take() {
        if !a.step.is_empty() {
            let received = store.take_interventions(&item.id);
            if !received.is_empty() { run.interventions = received; }
            let out = Outcome {
                ok: true,
...
mod tests {
    ...
    #[tokio::test]
    async fn legacy_human_answer_resumes_once_and_consumes_the_stored_answer() {
        let Some((repo, base)) = sandbox("legacy-answer").await else { return };
        ...
        let p = Pipeline { steps: vec![
            step("already", StepKind::Shell { command: "echo duplicated > duplicate.txt".into() }, vec![]),
            step("ask", StepKind::Human { question: "continue?".into() }, vec![Flow::to("finish", super::super::pipeline::Cond::Always)]),
            step("finish", StepKind::Shell { command: "echo done > finished.txt".into() }, vec![]),
        ], ..Default::default() };
        let mut run = fresh_run(&base);
        run.ask = Some(super::super::model::Ask { at: 1, question: "continue?".into(), options: vec![], iteration: 1, step: "ask".into() });
        run.pipeline_next = Some("already".into());
        run.interventions = vec!["approved".into()];
        store.put_run(run.clone());
        run_pipeline(store.clone(), item, p, run, base.clone(), |_| {}).await;
        let run = store.run("t").unwrap();
        assert_eq!(run.state, RunState::Done, "{}", run.stop_note);
        assert_eq!(run.vars["ask"].output, "approved");
        assert!(run.interventions.is_empty());
