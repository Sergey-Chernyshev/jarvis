...
use super::manifest_v2::{
    validate_bundled_schema, validate_packaged_manifest, validate_source_manifest,
    HostCompatibility, Target,
};

#[derive(Clone, Debug)]
pub struct HostPackageDocumentAdapter {
    compatibility: HostCompatibility,
}

impl HostPackageDocumentAdapter {
    pub fn new(compatibility: HostCompatibility) -> Self {
        Self { compatibility }
    }

    fn target(target: PackageTarget) -> Target {
        match target {
            PackageTarget::DarwinArm64 => Target::darwin_arm64(),
            PackageTarget::DarwinAmd64 => Target::darwin_amd64(),
        }
    }

    fn validate_schema(schema: &[u8], canonical_bytes: &[u8]) -> Result<(), PackageError> {
        let value: serde_json::Value = serde_json::from_slice(canonical_bytes)
            .map_err(|_| PackageError::package_metadata())?;
        validate_bundled_schema(schema, &value).map_err(|_| PackageError::package_metadata())
    }
}

impl PackageDocumentAdapter for HostPackageDocumentAdapter {
    fn resolve_source_manifest(
        &self,
        spooled_bytes: &[u8],
        target: PackageTarget,
    ) -> Result<ManifestV2, PackageError> {
        validate_source_manifest(spooled_bytes, &Self::target(target), &self.compatibility)
    ...
    fn validate_packaged_manifest(
        ...
        validate_packaged_manifest(canonical_bytes, &Self::target(target), &self.compatibility)
            .map_err(PackageError::manifest)
    }

    fn validate_package_metadata_schema(&self, canonical_bytes: &[u8]) -> Result<(), PackageError> {
        Self::validate_schema(PACKAGE_METADATA_SCHEMA_JSON, canonical_bytes)
    }
    ...
    fn validate_package_signature_schema(
        ...
        Self::validate_schema(PACKAGE_SIGNATURE_SCHEMA_JSON, canonical_bytes)
    }
}
...
mod tests {
    ...
    fn real_adapter_resolves_source_target_and_revalidates_packaged_manifest() {
        ...
        let ui = adapter
            .resolve_source_manifest(VALID_UI, PackageTarget::DarwinArm64)
            .unwrap();
        let concrete_ui = serde_json::to_vec(&ui).unwrap();
        assert_eq!(
            adapter
                .validate_packaged_manifest(&concrete_ui, PackageTarget::DarwinArm64)
                .unwrap(),
            ui
        );
    }
    ...
    fn real_adapter_validates_closed_metadata_and_signature_schemas() {
        ...
        adapter
            .validate_package_signature_schema(&serde_json::to_vec(&signature).unwrap())
            .unwrap();

        let mut unknown = metadata;
        unknown["escapeSandbox"] = json!(true);
        assert!(adapter
            .validate_package_metadata_schema(&serde_json::to_vec(&unknown).unwrap())
            .is_err());
    }

    #[test]
    fn bundled_package_schemas_have_no_remote_references() {
        for schema in [PACKAGE_METADATA_SCHEMA_JSON, PACKAGE_SIGNATURE_SCHEMA_JSON] {
