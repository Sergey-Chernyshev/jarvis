...
use crate::plugin_packages::trust::package::SharedPackageVerifier;
...
// Keep the audited journal payload representation during the service extraction.
...
impl PackageManagerApi for PluginManager {
    ...
    fn rollback(&self, id: &PluginId, version: Option<&Version>) -> ManagerResult<InstallReceipt> {
        ...
        let previous = current
            .previous
            .as_ref()
            .filter(|receipt| version.is_none_or(|version| &receipt.version == version))
    ...
    fn doctor(&self, id: Option<&PluginId>) -> ManagerResult<DoctorReport> {
        let recoverable_operations = self
            ...
            .recoverable()?
            .into_iter()
            .filter(|operation| id.is_none_or(|id| operation.plugin_id == id.as_str()))
            .collect::<Vec<_>>();
        let current = id
