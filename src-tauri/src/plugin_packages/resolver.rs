...
use super::verified_exec::VerifiedExecutable;

const LEGACY_AGENT_VM_ID: &str = "agent-vm";
#[cfg(test)]
const LEGACY_MANIFEST: &[u8] = b"test-only legacy bridge fixture";
...
#[derive(Clone, Debug)]
// Receipt and legacy evidence intentionally remain separate variants.
...
impl ResolvedPlugin {
    ...
    pub fn root(&self) -> &Path {
        match self {
            ...
            Self::LegacyAgentVm(plugin) => &plugin.root,
        }
    }
...
/// Injected separately from v2 receipt trust; no filesystem fallback is supplied.
pub trait LegacyResolver: Send + Sync {
    fn resolve(&self, paths: &PluginPaths, id: &PluginId) -> Result<PathBuf, ResolverError>;
}

pub struct PluginResolver {
    legacy: Option<Arc<dyn LegacyResolver>>,
    paths: PluginPaths,
...
impl PluginResolver {
    ...
    pub fn with_legacy_bridge(mut self, bridge: Arc<dyn LegacyResolver>) -> Self {
        self.legacy = Some(bridge);
        self
    }
    ...
    pub fn candidate_ids(&self) -> Vec<PluginId> {
        ...
        for entry in entries.flatten() {
            ...
            let is_legacy_agent_vm = self.legacy.is_some()
                && plugin_id.as_str() == LEGACY_AGENT_VM_ID
        ...
        let root = self
            ...
            .ok_or_else(|| {
                ResolverError::new(
                    ...
                    "no bundled legacy bridge is configured",
                )
            })?
            .resolve(&self.paths, plugin_id)?;
        match receipts.current(plugin_id) {
            Ok(None) => {}
            Ok(Some(_)) => {
                return Err(ResolverError::receipt_blocked(
...
mod tests {
    ...
    use crate::plugin_packages::manifest_v2::HostCompatibility;
    use crate::plugin_packages::package_manager::paths::PluginPaths;
    use crate::plugin_packages::package_manager::receipt::{ReceiptStore, VersionStore};
    ...
    struct FixtureLegacyBridge;
    impl super::LegacyResolver for FixtureLegacyBridge {
        fn resolve(
            &self,
            paths: &PluginPaths,
            id: &PluginId,
        ) -> Result<PathBuf, super::ResolverError> {
            let root = paths.plugin(id);
            if fs::read(root.join("manifest.json")).ok().as_deref() != Some(LEGACY_MANIFEST) {
                return Err(super::ResolverError::new(
    ...
    impl Fixture {
        ...
        fn resolver(&self) -> PluginResolver {
            ...
            .with_legacy_bridge(Arc::new(FixtureLegacyBridge))
        }
    ...
    #[test]
    fn default_resolver_never_admits_or_discovers_unreceipted_legacy_directories() {
        let fixture = Fixture::new("no-production-legacy");
        ...
        assert!(resolver.candidate_ids().is_empty());
        assert_eq!(
            resolver
