...
impl CatalogPackageVerifier {
    ...
    pub(crate) fn verify_persisted(
        ...
        let mut signature_message = Vec::with_capacity(
            crate::plugin_packages::trust::package_signature_domain().len()
                + 1
                + package_json.len(),
        );
        signature_message
            .extend_from_slice(crate::plugin_packages::trust::package_signature_domain());
...
mod tests {
    ...
    ) -> Result<VerifiedCatalog, crate::plugin_packages::trust::TrustError> {
    ...
    fn verified_release(
        catalog: Value,
        selection: &CatalogSelection,
    ) -> Result<VerifiedCatalogRelease, crate::plugin_packages::trust::TrustError> {
