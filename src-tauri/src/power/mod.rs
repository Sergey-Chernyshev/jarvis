...
impl Power {
    ...
    /// Держать машину бодрствующей, пока крутится цикл.
    ///
    /// Цикл работает ночью и без человека: уснувший мак — это оборванная
    /// итерация и утро без результата. Берём тот же ручной ассерт, что и
    /// «Не спать» из трея, — отдельной машинерии он не заслуживает.
    pub fn loop_running(&self, on: bool) {
        let mut engine = self.engine.lock().unwrap();
        let Some(e) = engine.as_mut() else { return }; // плагин «Не спать» выключен
        let _ = if on {
            e.start_manual(Some("цикл".into()))
        } else {
            e.stop_manual()
        };
    }
