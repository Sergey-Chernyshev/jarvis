...
fn defaults() -> Value {
    json!({
        ...
        // Потолок сессий, которые агент вправе держать одновременно через
        // sessions.spawn. Не в SETTINGS_ALLOWLIST намеренно: агент, который
        // вправе поднять себе потолок, потолка не имеет.
        "sessionsSpawnMax": 4,
        // Бюджет лимитов (budget.rs). Как и sessionsSpawnMax, НЕ в
        // SETTINGS_ALLOWLIST: агент, который вправе поднять себе потолок,
        // потолка не имеет. Проценты — доли недельного лимита.
        "budget": {
