...
(() => {
  ...
  function mount(els, api) {
    ...
    const provider = els.provider || document.createElement('select');
    if (!els.provider) {
      provider.setAttribute('aria-label', 'Провайдер агента');
      for (const [id, label] of [['auto', 'Авто'], ['claude', 'Claude'], ['codex', 'Codex']]) {
        const option = document.createElement('option'); option.value = id; option.textContent = label; provider.appendChild(option);
      }
      sub.parentElement?.insertBefore(provider, sub);
    }
    let ready = false, hasProvider = true;
    ...
    function syncHead() {
      const t = here();
      sendBtn.disabled = t.busy || !ready || !hasProvider;
      provider.disabled = t.busy || !ready;
      if (t.session && t.provider) provider.value = t.provider;
      ...
      sub.textContent = (cur ? cut(cur.name, 22) + ' · ' : '') + (t.busy ? 'думает…' : !ready ? 'подключение…' : hasProvider ? 'готов' : 'CLI не найден');
    ...
    const CHAIN_CARD = {
      // Раньше цепочка писала промпт прямо в сессию — «заход ушёл» было про это.
      // Теперь она будит Джарвиса, и он решает сам. Слово должно называть то,
      // что произошло, иначе человек не отличит новую конструкцию от старой.
      sent: 'Джарвис разбужен событием',
      proposed: 'заход предложен',
      failed: 'разбудить не вышло',
    ...
    function chainText(ev) {
      if (ev.kind === 'sent') return 'Заход ' + (ev.step || 0) + '. Что ему передали:\n' + (ev.prompt || '');
    ...
    async function listCmd(what, run) {
      ...
      for (const c of chats) {
        if (!c.id) continue;
        const t = thread(c.id);
        if (!t.busy) { t.session = c.sessionId || null; t.provider = c.provider || null; }
    ...
    async function send() {
      ...
      if (!text || t.busy || !ready || !hasProvider) return;
      ...
      try {
        ...
        res = await api.send(text, t.id, t.session, t.provider || selectedProvider());
        if (res?.provider) t.provider = res.provider;
    ...
    const eventsReady = api.onEvent((ev0) => {
      const ev = ev0 || {};
    ...
    const confirmsReady = api.onConfirm((c0) => {
      const c = c0 || {};
      const cd = c.card || {};
      ...
      const btns = el('cbtns');
      const yes = document.createElement('button'); yes.type = 'button'; yes.className = 'cbtn yes'; yes.textContent = 'Разрешить';
      const no = document.createElement('button'); no.type = 'button'; no.className = 'cbtn no'; no.textContent = 'Отклонить';
    ...
    provider.addEventListener('change', async () => {
      ...
      // A new provider starts a new conversation; the old chat remains recoverable.
      if (t.session) {
        const fresh = await listCmd('создать чат для выбранного агента', () => api.create());
        if (fresh) { afterPick(); await loadHistory(fresh.id); }
        else { provider.value = t.provider || 'auto'; syncHead(); return; }
      }
      here().provider = null;
      provider.value = choice;
      hasProvider = !provider.selectedOptions?.[0]?.disabled;
      syncHead();
      try { const result = await api.setProvider?.(selectedProvider()); if (result?.ok === false) throw new Error(result.error); }
      catch (error) { addErr(here(), 'Выбор действует в этом окне, но не сохранён: ' + errText(error)); }
    });
    Promise.all([eventsReady, confirmsReady, api.hosts?.()]).then(([, , hosts]) => {
      if (hosts?.ok === false) throw new Error(hosts.error || 'Не удалось проверить CLI');
      if (Array.isArray(hosts?.providers)) {
        for (const option of provider.options) {
          const host = hosts.providers.find(h => h.id === option.value);
          option.disabled = !host?.available;
          option.textContent = (host?.label || option.textContent) + (host?.available ? '' : ' · CLI не найден');
        }
        provider.value = hosts.selected || 'auto';
        hasProvider = !provider.selectedOptions?.[0]?.disabled;
      }
      ready = true; syncHead();
    }).catch(error => { addErr(here(), 'Не удалось подготовить агента: ' + errText(error)); });
    syncHead();
    input.focus();
    ...
    if (!root.dataset.mounted) {
      ...
      tab = mount(
        ...
        },
        {
          ...
          send: (message, chatId, sessionId, provider) => j.agentSend(message, chatId, sessionId, provider),
          hosts: () => j.agentHosts?.(),
          setProvider: (provider) => j.setSettings({ agentProvider: provider }),
  ...
  if (document.getElementById('msgs')) {
    ...
    mount(
      { provider: g('provider'), msgs: g('msgs'), input: g('input'), sendBtn: g('send'), sub: g('sub'), tag: g('tag'), newBtn: g('newChat'), chatsRow: g('chats') },
      {
        ...
        send: (message, chatId, sessionId, provider) => invoke('agent_send', { message, chatId, sessionId, provider }),
        hosts: () => invoke('agent_hosts'),
        setProvider: (provider) => invoke('settings_set', { patch: { agentProvider: provider } }),
