...
(() => {
  ...
  listen("agent:event", (e) => {
    ...
    switch (ev.type) {
      ...
      case "done":
        ...
        if (ev.result && (!curBubble || !curBubble.textContent))
          startBot().textContent = ev.result;
        setBusy(false);
        curBubble = null;
        break;
    }
  ...
  listen("agent:confirm", (e) => {
    ...
    if (c.provenance === "untrusted")
      box.appendChild(el("cwarn", "⚠ данные из недоверенного источника"));
