...
test('после перезапуска окна разговор продолжается, а не начинается заново', async () => {
  ...
  await say('привет');
  assert.deepEqual(sent(), [{ message: 'привет', chatId: null, sessionId: 's-42', provider: 'auto' }]);
});
...
test('без сохранённого разговора окно молчит про продолжение', async () => {
  ...
  await say('привет');
  assert.deepEqual(sent(), [{ message: 'привет', chatId: null, sessionId: null, provider: 'auto' }]);
});
