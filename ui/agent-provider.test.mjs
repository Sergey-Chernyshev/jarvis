...
function fixture({ selected = 'auto', claude = true, codex = true, subscribe, createResult, confirmResult = { ok: true } } = {}) {
  ...
  window.__TAURI__ = {
    core: { invoke: async (name, args) => {
      ...
      if (name === 'agent_chat_create') return createResult || { ok: true, current: 'new-provider', chats: [{ id: 'new-provider', name: 'New chat', sessionId: null }] };
      return { ok: true };
    } },
    event: { emit: async () => {}, listen: (name, callback) => {
      listeners[name] = callback;
      return subscribe || Promise.resolve(() => {});
    } },
  };
  new Function('window', 'document', read('./icons.js'))(window, document);
  new Function('window', 'document', read('./markdown.js'))(window, document);
...
test('confirmation buttons are keyboard controls and never report approval for an expired request', async () => {
  ...
  yes.click();
  await flush();
  assert.equal(f.calls.some(c => c.name === 'agent_confirm'), false, 'unarmed click cannot authorize');
  await humanPress(f.window, yes.closest('.cbox'), yes);
  await flush();
  assert.match(f.document.querySelector('.cresult').textContent, /время вышло|завершён|истёк/);
  assert.doesNotMatch(f.document.querySelector('.cresult').textContent, /разрешено/);
});


test('failed provider switch preserves the current conversation and provider', async () => {
  const f = fixture({ selected: 'codex', createResult: { ok: false, error: 'disk unavailable' } });
  await flush(); f.send('hello'); await flush();
  f.emit({ type: 'init', session_id: 'same-session' });
  f.emit({ type: 'done', session_id: 'same-session' });
  f.provider.value = 'claude'; f.provider.dispatchEvent(new f.window.Event('change'));
  await flush(); f.send('continue'); await flush();
  const last = f.calls.filter(c => c.name === 'agent_send').at(-1).args;
  assert.equal(last.sessionId, 'same-session'); assert.equal(last.provider, 'codex');
  assert.equal(f.calls.some(c => c.name === 'settings_set'), false);
});
