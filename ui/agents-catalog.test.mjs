...
test('панель не выбирает модели и усилие тернарником по id', () => {
  ...
  assert.ok(!/s\.agent === 'codex'/.test(renderer.slice(renderer.indexOf('const EFFORT_SHORT'), renderer.indexOf('const EFFORT_SHORT') + 2200)), 'в палитре остался хардкод codex');
