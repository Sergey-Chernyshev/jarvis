...
(() => {
  ...
  window.jarvis = {
    agentInstancesList: () => invoke('agent_instances_list'),
    agentInstancesSave: (config) => invoke('agent_instances_save', { config }),
    agentInstancesRepair: (instanceIds) => invoke('agent_instances_repair', { instanceIds }),
    remotesRepairSource: (name, sourceId) => invoke('remote_source_repair', { name, sourceId }),
    meetingsList: () => invoke('meetings_list'),
    meetingsStatus: () => invoke('meetings_status'),
    meetingsSources: () => invoke('meetings_sources'),
    meetingsStart: (title, source) => invoke('meetings_start', { title, source }),
    meetingsStop: () => invoke('meetings_stop'),
    meetingsGet: (id) => invoke('meetings_get', { id }),
    meetingsRetranscribe: (id) => invoke('meetings_retranscribe', { id }),
    onMeetingsChanged: (cb) => on('meetings_changed', cb),
    copyText: (text) => invoke('plugin:clipboard-manager|write_text', { text: String(text) }),
    onState: (cb) => on('state', cb),
    onShown: (cb) => on('panel-shown', () => cb()),
    onOpenSession: (cb) => on('open-session', cb),
    onWorkspaceRoute: (cb) => on('workspace-route', cb),
    openWorkspace: (options = {}) => invoke('workspace_open', { options }),
    ...
    teleportStatus: (proxy) => invoke('teleport_status', { proxy: proxy || null }),
    teleportNodes: (proxy, cluster) => invoke('teleport_nodes', { proxy: proxy || null, cluster: cluster || null }),
    teleportLogin: (proxy, renew = false) => invoke('teleport_login', { proxy, renew }),
    ...
    remotesPreflight: (sshHost, jarvisDir, connection) => invoke('remotes_preflight', { sshHost, jarvisDir, connection: connection || null }),
    ...
    remotesSshAuthorize: (sshHost, password, connection) => invoke('remotes_ssh_authorize', { sshHost, password, connection: connection || null }),
    ...
    continueSession: (sessionId) => invoke('session_continue_managed', { sessionId }),
    machinesList: () => invoke('machines_list'),
    projectsList: (machine) => invoke('projects_list', { machine: machine || null }),
    projectsIconCandidates: (machine, cwd) => invoke('projects_icon_candidates', { machine, cwd }),
    projectsSave: (project) => invoke('projects_save', { project }),
    projectsRemove: (machine, cwd) => invoke('projects_remove', { machine, cwd }),
    vmStatus: () => invoke('vm_status'),
    vmAction: (name, action) => invoke('vm_action', { name, action }),
    ...
    fileDialogState: active => invoke('file_dialog_state', { active }),
    readArtifact: (sessionId, path) => invoke('artifact_read', { sessionId, path }),
    artifactNotes: (key, note = null, removeId = null) => invoke('artifact_notes', { key, note, removeId }),
    saveAttachment: (dataBase64, name, machine) => invoke('session_save_attachment', { dataBase64, name, machine }),
    saveImage: (dataBase64, ext) => invoke('session_save_image', { dataBase64, ext }),
    pingTerminal: (sessionId) => invoke('terminal_ping', { sessionId }),
    terminalSnapshot: (sessionId) => invoke('session_terminal_snapshot', { sessionId }),
    terminalAction: (sessionId, action, payload = {}) => invoke('session_terminal_action', { sessionId, action, payload }),
    terminalKey: (sessionId, key) => invoke('session_terminal_key', { sessionId, key }),
    onLaunchTask: (cb) => on('session:launch-task', cb),
    onProjectsChanged: (cb) => on('projects_changed', cb),
    ...
    analyticsConfig: () => invoke('analytics_config_get'),
    analyticsSaveConfig: (config) => invoke('analytics_config_save', { config }),
    analyticsDefaults: () => invoke('analytics_config_defaults'),
    ...
    transcriptUpdate: (id, text) => invoke('transcript_update', { id, text }),
    ...
    dictionaryGet: () => invoke('dictionary_get'),
    dictionaryAdd: (word, replacement) => invoke('dictionary_add', { word, replacement }),
    dictionaryRemove: (word) => invoke('dictionary_remove', { word }),
    scratchpadGet: () => invoke('scratchpad_get'),
    scratchpadSet: (text) => invoke('scratchpad_set', { text }),
