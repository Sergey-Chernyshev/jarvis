...
test("main UI loads repair classifier before settings and exposes the proxy CTA contract", async () => {
  ...
  const repairIndex = html.indexOf(
    '<script src="./download-repair.js"></script>',
  );
  const checkboxIndex = html.indexOf(
    '<script src="./model-checkbox.js"></script>',
  );
  ...
  assert.notEqual(
    checkboxIndex,
    -1,
    "main UI loads the model checkbox control",
  );
  assert.ok(
    checkboxIndex < settingsIndex,
    "model checkbox control loads before settings",
  );
...
test("fresh model progress authoritatively clears its stale error before rendering", async () => {
  ...
  const progressStart = settings.indexOf(
    "window.jarvis.onModelInstallProgress",
  );
  ...
  assert.ok(
    ...
    "error clears before progress is rendered",
  );

  const legacyProgressStart = settings.indexOf(
    "window.jarvis.onSttInstallProgress",
  );
  assert.notEqual(
    legacyProgressStart,
    -1,
    "legacy progress subscription exists",
  );
  const legacyProgressBlock = settings.slice(
    legacyProgressStart,
    legacyProgressStart + 650,
  );
  assert.match(
    legacyProgressBlock,
