...
/* Отказ при непустом остатке. Бюджет отказывает по СВОБОДНОМУ числу — остаток
 * минус резерв на последний день минус занятое под уже разрешённую работу, — а
 * футер показывал голый остаток. Человек видел «осталось 30%» и получал отказ:
 * цифра, по которой нельзя предсказать поведение, хуже отсутствия цифры. */
test('крупно стоит свободное, а не голый остаток', () => {
  const l = line('claude', claude({ weekLeftPct: 30, availablePct: 15.2, reservePct: 12.8, reservedPct: 2 }), MON);
  assert.equal(l.free, 15, 'наружу ушёл не тот процент, по которому отказывают');
  assert.notEqual(l.free, l.pct, 'свободное и остаток совпали — разбивку не видно');
});

test('из чего сложился остаток — видно в подсказке', () => {
  const l = line('claude', claude({ weekLeftPct: 30, availablePct: 15.2, reservePct: 12.8, reservedPct: 2 }), MON);
  assert.match(l.title, /остаток 30% = свободно 15%/);
  assert.match(l.title, /резерв на последний день 13%/);
  assert.match(l.title, /занято под уже разрешённое 2%/);
});

test('нечего разбивать — разбивки и нет', () => {
  // Резерва и броней нет: «остаток = свободно» — шум, а не объяснение.
  const l = line('claude', claude({ weekLeftPct: 44.3, availablePct: 44.3, reservePct: 0, reservedPct: 0 }), MON);
  assert.equal(l.title.includes('остаток'), false, 'подсказка объясняет то, чего не происходит');
  assert.equal(l.free, 44);
});
