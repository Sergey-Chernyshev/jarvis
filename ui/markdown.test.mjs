...
test("список: отступный перенос — продолжение пункта", () => {
  assert.equal(
    MD.render("- длинный пункт\n  и его перенос"),
    "<ul><li>длинный пункт и его перенос</li></ul>",
  );
});
...
test("цитата: рекурсивный рендер содержимого", () => {
  assert.equal(
    MD.render("> цитата\n> **жирная**"),
    "<blockquote><p>цитата <strong>жирная</strong></p></blockquote>",
  );
});
...
test("жирный/курсив/код", () => {
  assert.equal(
    MD.render("**ж** *к* `код`"),
    "<p><strong>ж</strong> <em>к</em> <code>код</code></p>",
  );
});
...
test("кавычка в url не вырывается из атрибута data-href", () => {
  ...
  assert.ok(
    html.includes('data-href="https://e.com/&quot;onmouseover=&quot;x"'),
    html,
  );
  assert.ok(
    !/"\s*onmouseover\s*=/.test(html.replace(/&quot;/g, "")),
    "атрибут не инжектится",
  );
});
...
test('http(s)-ссылка — <a class="md-link"> с data-href, без настоящего href', () => {
  ...
  assert.equal(
    html,
    '<p><a class="md-link" data-href="https://example.com/x">доки</a></p>',
  );
  assert.ok(
    !html.includes(" href="),
    "настоящий href не ставим — навигация вебвью запрещена",
  );
});
...
test("относительная ссылка — текст с классом, не кликабельна", () => {
  ...
  assert.equal(
    html,
    '<p><span class="md-link-rel" title="docs/design.md">спека</span></p>',
  );
