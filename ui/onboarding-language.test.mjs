...
async function boot(patch) {
  ...
  for (const name of ['onboarding-state.js', 'onboarding.js']) {
    ...
    fn(window, document, window, { userAgent: 'Macintosh; Mac OS X' }, setTimeout, window.requestAnimationFrame);
  }
...
test('экран отказа говорит, что произошло и что нажать — без внутренних слов', async () => {
  ...
  const text = doc.getElementById('content').textContent;
  assert.ok(text.includes('Не получилось скачать'), 'причина не названа: ' + text);
  assert.ok(text.includes('Повторить'), 'не сказано, что нажать');
