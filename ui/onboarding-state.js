(function (root, factory) {
  ...
  function classifyFailure(failures) {
    ...
    if (/network|proxy|timeout|timed out|http|dns|соедин|сеть/.test(text))
      return "network";
    if (
      /no space|disk|device full|readonly|read-only|места на диске/.test(text)
    )
      return "disk";
    if (/permission|denied|trust|access|доступ|прав/.test(text))
      return "permission";
    if (/\bhome\b|\bpath\b|environment|окружен/.test(text))
      return "environment";
  ...
  function derive(snapshot) {
    ...
    const socket = (Array.isArray(data.transport) ? data.transport : []).find(
      (item) => item && item.id === "socket",
    );
    const readyCapabilities = capabilities.filter(
      (item) => item && item.ready,
    ).length;
    const base = {
      snapshot: data,
      readyCapabilities,
      ...
      failureKind: null,
      runtimeState: data.coreReady
        ? socket && socket.ready
          ? "online"
          : "warming"
        : "offline",
      readyAgents: agents.filter((item) => item && item.ready).length,
      totalAgents: agents.filter((item) => item && item.available !== false)
        .length,
      configHealth,
      configIssues: Array.isArray(configHealth.issues)
        ? configHealth.issues.slice()
        : [],
    };
  ...
  function selectedPlan(selection) {
    ...
    if (value.qwen)
      ids.push(value.qwenSize === "qwen3-1.7b" ? "qwen3-1.7b" : "qwen3-0.6b");
