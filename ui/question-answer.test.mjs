...
test("multi-select without toggles and text is rejected", () => {
  assert.equal(
    QA.commitRow({ multiSelect: true, chosen: new Set(), sel: 0, text: "  " }),
    null,
  );
});
...
test("payload without customs is byte-for-byte the legacy contract", () => {
  assert.deepEqual(QA.buildPayload([[2], [1]], [null, null]), {
    answers: [[2], [1]],
  });
...
test("payload carries texts aligned to questions when any custom present", () => {
  assert.deepEqual(QA.buildPayload([[], [1, 3]], ["свой ответ", null]), {
    answers: [[], [1, 3]],
    texts: ["свой ответ", null],
  });
});
