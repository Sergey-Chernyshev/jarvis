...
function setView(next) {
  ...
  view = next;
  // Отметка самого перехода: без неё отсутствие следов в логе неотличимо от
  // того, что нажатие вообще не доехало до этой функции.
  try { window.jarvis.reportError('view', `переход → ${next}`); } catch (_) { /* лог не обязателен */ }
  closeActions();
  ...
  const safely = (what, fn, host) => {
    ...
    const audit = () => {
      ...
      // Пишем ВСЕГДА, а не только на беде. Молчание означало сразу две вещи —
      // «всё хорошо» и «нажатие не дошло до сюда», — а это ровно те два случая,
      // которые надо различать в первую очередь. Строка на переход в раздел
      // стоит дёшево: переходы редки.
      try {
