...
(() => {
  ...
  window.toast = Object.freeze({
    onAdd: (cb) => {
      listen("toast-add", (e) => cb(e.payload)).then(armed);
    },
    onUpdate: (cb) => {
      listen("toast-update", (e) => cb(e.payload)).then(armed);
    },
    ...
    onHover: (cb) => {
      listen("toast-hover", (e) => cb(e.payload));
    },
    // голос держит карточку, пока говорит (hold), и продлевает на N мс после (extend)
    onHold: (cb) => {
      listen("toast-hold", (e) => cb(e.payload));
    },
    onExtend: (cb) => {
      listen("toast-extend", (e) => cb(e.payload));
    },
    // снять карточку по id (вопрос ответили) — не через armed(), это не буфер
    onRemove: (cb) => {
      listen("toast-remove", (e) => cb(e.payload));
    },
    click: (sessionId, target) =>
      invoke("toast_click", {
        sessionId,
        target: target || null,
      }),
    ...
    onVoiceHud: (cb) => {
      listen("voice-hud", (e) => cb(e.payload));
    },
    onAudioState: (cb) => {
      listen("audio_state", (e) => cb(e.payload));
    },
    // тап по варианту пикера (sessionId=null → отмена выбора) и «Отменить» стейджа
    voicePick: (nonce, sessionId) =>
    ...
    voiceConfirm: (nonce, approved) =>
      invoke("voice_confirm_resolve", { nonce, approved }),
