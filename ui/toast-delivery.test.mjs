...
test('мёртвая пана: карточка вопроса остаётся и называет причину', async () => {
  ...
  assert.equal(doc.querySelectorAll('[role="status"]').length, 1, 'причина отказа не показана');
  assert.ok(doc.querySelector('[role="status"]').textContent.includes('Пана сессии не отвечает'));
});

test('удачный ответ по-прежнему снимает карточку', async () => {
  const { doc, handlers, calls } = boot({ answerQuestion: { ok: true, delivery: 'confirmed' } });
  handlers.add(QUESTION);
  await settle();
  ...
  assert.equal(calls[0][0], 'answerQuestion'); assert.equal(calls[0][1], 's1'); assert.deepEqual(calls[0][2].answers, [[2]]); assert.ok(calls[0][2].submissionId);
