...
window.toast.onAdd((d) => {
  ...
  const configuredTtl =
    Number.isFinite(d.ttlMs) && d.ttlMs >= 0 ? d.ttlMs : TTL;
...
function renderVoiceHud(p) {
  ...
  dot.className =
    "dot" +
    (["staged", "picker", "confirm"].includes(p.phase) ? " waiting" : "");
  const title = document.createElement("div");
  title.className = "title";
  // staged: показываем КУДА уйдёт промпт — это и есть смысл окна отмены (VR-2)
  if (p.phase === "staged" && p.label)
    title.textContent = `Отправлю → ${p.label}`;
  ...
  if (p.phase === "staged") {
    ...
    copy.addEventListener("click", (e) => {
      e.stopPropagation();
      try {
        window.toast.copy(p.full || p.body || "");
      } catch {}
