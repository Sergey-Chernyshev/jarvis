...
(function () {
  ...
  const BUILTIN_PROMPTS = [
    ...
    },
    {
      id: "commit",
      icon: "git",
      name: "Коммит-сообщение",
      desc: "Делает аккуратный git-commit: заголовок + тело.",
      trigger: "текст похож на описание изменения",
      auto: true,
      enabled: true,
    },
    {
      ...
      trigger: "личные сообщения и заметки",
      auto: true,
      enabled: true,
    },
    {
  ...
  async function ipcGetTranscripts() {
    try {
      if (!window.jarvis || typeof window.jarvis.transcriptsGet !== "function")
        return [];
      ...
      return (
        items
          .map((it, i) => ({
            ...
            appliedStyle:
              (it && typeof it.appliedStyle === "string" && it.appliedStyle) ||
              null,
          }))
  ...
  async function ipcEnhance(text, style) {
    try {
      if (
        !window.jarvis ||
        typeof window.jarvis.transcriptEnhance !== "function"
      ) {
  ...
  function hasDelete() {
    return !!(
      window.jarvis && typeof window.jarvis.transcriptDelete === "function"
    );
  }
  ...
  async function ipcDictGet() {
    try {
      if (!window.jarvis || typeof window.jarvis.dictionaryGet !== "function")
        return null;
  ...
  async function ipcDictRemove(word) {
    try {
      if (
        window.jarvis &&
        typeof window.jarvis.dictionaryRemove === "function"
      ) {
  ...
  async function ipcPromptsGet() {
    try {
      if (!window.jarvis || typeof window.jarvis.promptsGet !== "function")
        return null;
      ...
      return prompts
        .map((p) => ({
          ...
          trigger:
            p && typeof p.trigger === "string" && p.trigger ? p.trigger : null,
          auto: !!(p && p.auto),
  ...
  async function ipcPromptsSettings() {
    try {
      if (
        !window.jarvis ||
        typeof window.jarvis.promptsGetSettings !== "function"
      )
        return null;
  ...
  async function ipcPromptsSetSmart(on) {
    try {
      if (
        window.jarvis &&
        typeof window.jarvis.promptsSetSmart === "function"
      ) {
  ...
  async function ipcScratchGet() {
    ...
    try {
      const v = window.localStorage
        ? window.localStorage.getItem(SCRATCH_KEY)
        : null;
  ...
  async function ipcScratchSet(text) {
    ...
    try {
      if (window.localStorage)
        window.localStorage.setItem(SCRATCH_KEY, String(text));
    } catch (e) {
  ...
  function buildEntry(item) {
    ...
    transformBtn.addEventListener("click", (e) => {
      ...
      if (
        openMenu &&
        openMenu.parentNode === entry &&
        openMenu.classList.contains("vh-tmenu") &&
        !openMenu.classList.contains("vh-ovf")
      ) {
        closeMenu();
      } else {
    ...
    if (
      item.hasAudio &&
      window.jarvis &&
      typeof window.jarvis.transcriptRetranscribe === "function"
    ) {
      ...
      regenBtn.addEventListener("click", async (e) => {
        ...
        let r = null;
        try {
          r = await window.jarvis.transcriptRetranscribe(item.id);
        } catch {}
  ...
  function renderHistory() {
    ...
    if (!items.length) {
      ...
      if (state.items.length && state.query) {
        ...
        empty.textContent =
          "Пока пусто. Надиктуй что-нибудь через диктовку (Whisper · F8).";
      }
    ...
    for (const it of items) {
      ...
      if (k !== curKey) {
        ...
        head.appendChild(
          el("span", "vh-cnt", cntN + " " + pluralReplicas(cntN)),
        );
  ...
  function renderRail() {
    ...
    rail.appendChild(
      makeStatCard(fmtNum(words), "всего слов", { accent: true }),
    );
    ...
    if (state.usage) {
      ...
      if (tok != null) {
        const card = makeStatCard(
          fmtNum(Math.round(Number(tok))),
          "токенов сегодня",
          { accent: false },
        );
        if (cost != null) {
  ...
  function buildInsightsPane() {
    ...
    grid.appendChild(
      buildBigCard(fmtNum(all.length), "записей в истории", [
    ...
    if (state.usageAll) {
      ...
      if (tok != null) {
        const rows = [];
        if (cost != null)
          rows.push({ k: "стоимость", v: "$" + Number(cost).toFixed(2) });
        grid.appendChild(
          buildBigCard(fmtNum(Math.round(Number(tok))), "токенов всего", rows),
        );
      }
    }
    ...
    h1.appendChild(
      el(
        "span",
        "cap",
        days.size + " " + pluralDays(days.size) + " с диктовкой",
      ),
    );
    ...
    if (!state.dict.length) {
      list.appendChild(
        el(
          "div",
          "vh-empty",
          "Пока нет своих слов — добавь термины, имена, названия.",
        ),
      );
      return;
    }
    ...
    if (!state.dictLive) {
      sect.appendChild(
        el(
          "div",
          "vh-note",
          "Скоро — словарь свяжется с движком распознавания. Пока список живёт в этой сессии.",
        ),
      );
    }
  ...
  function buildTransformsPane() {
    ...
    sMid.appendChild(
      el(
        "div",
        "tdesc",
        "Авто-подбор и применение преобразования по тексту и контексту.",
      ),
    );
    ...
    smartTg.addEventListener("click", async (e) => {
      ...
      if (state.smartTg && state.smartTg !== smartTg)
        state.smartTg.classList.toggle("off", !state.smart);
  ...
  function buildScratchPane() {
    ...
    ta.placeholder =
      "Надиктуй сюда что угодно — свободное поле для мыслей, заметок, длинных промптов…";
    ...
    hint.appendChild(
      document.createTextNode(" Зажми F8 и говори — текст добавится сюда"),
    );
  ...
  function buildSidebar() {
    ...
    fit.addEventListener("click", (e) => {
      ...
      try {
        if (
          window.jarvis &&
          typeof window.jarvis.onboardingOpen === "function"
        ) {
  ...
  async function initVoiceHistory(rootEl) {
    ...
    const [items, usage, usageAll, dict, prompts, settings, scratch] =
      await Promise.all([
        ipcGetTranscripts(),
        ipcUsage("today"),
        ipcUsage("all"),
    ...
    if (dict) {
      ...
      state.dict = [];
      state.dictLive = false;
    }

    state.prompts = prompts || BUILTIN_PROMPTS.map((p) => Object.assign({}, p));

    if (settings && typeof settings.smart === "boolean")
      state.smart = settings.smart;
    else if (settings && typeof settings.enabled === "boolean")
      state.smart = settings.enabled;
